...
const FIELDS_DOC =
  'https://docs.gpustack.ai/latest/user-guide/model-deployment-management/#import-deployments';
...
const ImportYamlModal: React.FC<ImportYamlModalProps> = ({
  ...
  // How many entries the pane's index was chosen against. An index only names
  // a deployment for as long as the document holds the same number of them.
  const plannedCount = useRef(0);
  ...
  // The cluster to check against: the one picked, or the default standing in
  // until someone picks. Derived rather than seeded into state, so a cluster
  // list that arrives after the drawer has mounted needs nothing synced —
  // the Select and the status line both read this.
  const defaultClusterId =
    clusterList.find((cluster) => cluster.is_default)?.value ??
    clusterList[0]?.value;
  const clusterId = picked ?? defaultClusterId;
  ...
  const reviewDocument = async (
    ...
    if (!hasDocument(text) || cluster == null) {
      requestId.current += 1;
    ...
    try {
      const result = await importModels(
        { content: text, cluster_id: cluster, dry_run: true },
      ...
      // The pane is keyed by position, so an edit that changes how many
      // documents there are re-numbers every entry after it: a `---` typed
      // into one pane leaves it holding two deployments while its index names
      // only the first, and the next edit patches both back in beside the
      // entry the second has already become. The document carries the edit by
      // now, so starting the pane over from it loses nothing and shows where
      // the new entry went.
      if (result.entries.length !== plannedCount.current) {
        plannedCount.current = result.entries.length;
      ...
      select(next);
      // Reseed when the document was just imported, when the entries moved
      // under the pane, or when a problem moved the selection: either way
      // what the buffer holds is not what the pane is now meant to be
      // showing. An ordinary re-plan leaves it alone, and so does one that
      // landed in a pane being typed in. The flag is cleared either way, so
      // it cannot stay armed and swap the buffer out under a later edit.
  ...
  const flushEdit = () => {
    ...
    flushedRef.current = text;
    // One entry is patched into the document where it sits, leaving the rest
    // of the file untouched; the whole document simply is the document. An
    // edit can also empty the document out from under the pane, leaving the
    // selection past the end of what is left — with no entry there to
    // replace, what is in the pane is the whole of what the user has.
    const next =
      typeof selected === 'number'
        ? (replaceDocumentAt(contentRef.current, selected, text) ?? text)
        : text;
  ...
  const handleClusterChange = (value: number) => {
    ...
    if (hasDocument(contentRef.current)) {
      reviewDocument(contentRef.current, value);
    }
  };
        ...
        <Flex vertical gap={12}>
          <Flex align="center" gap={16} wrap>
            {/* Not a form: one field, nothing submitted from it, and the
                ...
                a value, since the default stands in until one is picked. */}
            <SealSelect
              style={{ width: 280 }}
