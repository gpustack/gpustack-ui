...
// The document with one entry replaced by the text of the pane it was edited
// in, leaving every other entry byte for byte as it was. `null` when the
// index is past the end: the pane then belongs to a plan the document has
...
export const replaceDocumentAt = (
  text: string,
  index: number,
  next: string
): string | null => {
  ...
  documents[index] = index === 0 ? `${trimmed}\n` : `\n${trimmed}\n`;
